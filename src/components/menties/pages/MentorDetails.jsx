      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-5 lg:gap-6 xl:grid-cols-[minmax(0,300px)_minmax(0,1fr)]">
          ...
          <section className="space-y-5 sm:space-y-6">
            {/* Loading/Error State */}
            {(loading || error) && (
              <div className={`flex items-center gap-3 rounded-xl p-4 ${error ? 'bg-red-50 ring-1 ring-red-100' : 'bg-white ring-1 ring-[#e5e7eb]'
                }`}>
                {loading && (
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div className="flex items-center gap-0.5">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`h-4 w-4 ${i < Math.floor(item?.rating || 0)
                                  ? 'fill-[#f59e0b] text-[#f59e0b]'
                                  : 'text-[#e5e7eb]'
                                }`}
                            />
                  ...
                  <div className="rounded-xl bg-white p-4 ring-1 ring-[#e8dcff]">
                    <div className="flex items-center gap-0.5">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`h-4 w-4 ${i < Math.floor(review?.rating || 0)
                              ? 'fill-[#f59e0b] text-[#f59e0b]'
                              : 'text-[#e5e7eb]'
                            }`}
                        />
