...
export const authApi = {
  ...
  sendPasswordResetOtp: (payload) => apiClient.post('/auth/password-reset/send-otp/', payload, { auth: false }),
  verifyPasswordResetOtp: (payload) => apiClient.post('/auth/password-reset/verify-otp/', payload, { auth: false }),
  confirmPasswordReset: (payload) => apiClient.post('/auth/password-reset/confirm/', payload, { auth: false }),
  verifyMobileLoginOtp: (payload) => apiClient.post('/auth/mobile-login/verify-otp/', payload, { auth: false }),
  getLocationStates: () => apiClient.get('/locations/states/', { auth: false }),
  getLocationCities: (stateName) =>
    apiClient.get(`/locations/cities/?state=${encodeURIComponent(stateName || '')}`, { auth: false }),
